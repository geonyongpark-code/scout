...
// ---------- 플랫폼 담당 고정 명단 ----------
// 이 명단이 UID 접두어 판별(기획서 3-3)보다 우선한다. 명단에 없는 이름은 UID 접두어로 판별한다.
// 담당자가 바뀌면 이 명단만 수정하면 된다.
// 주의: 플랫폼이 "상품이 올라간 사이트"가 아니라 "검수자 소속"을 뜻하게 된다.
//       무신사 UID(5·6번대) 상품을 29cm 담당자가 검수하면 29cm 로 집계된다.
const PLATFORM_BY_REVIEWER = {
  "이온유": "무신사", "김혜련": "무신사", "이용준": "무신사", "김영조": "무신사", "윤미지": "무신사",
  "양호진": "29cm", "이재승": "29cm",
};
...
async function parseMessage(msg, agg) {
  ...
  for (const g of groups) {
    ...
    for (const uid of g.uids) {
      const plat = PLATFORM_BY_REVIEWER[reviewer] || (uid[0] === "4" ? "29cm" : "무신사");
