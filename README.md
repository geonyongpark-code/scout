...
- **일자별 추이**: `전체`(회색 점선) / `무신사`(파랑) / `29cm`(주황) 3개 선. 범례를 클릭하면 계열을 켜고 끌 수 있다. 수정요청이 없던 날도 0으로 표시해 날짜 간격이 왜곡되지 않는다.
- **1차검수자별 오류량 / 오류 항목별 빈도**: 막대를 클릭하면 아래 원본 드릴다운이 그 조건으로 필터된다.
- **검수자 × 항목 교차**(기획서 4-6): 행=검수자, 열=오류 항목 히트맵. 색이 진할수록 많다(빨강 계열 6단계). 검수자 전원을 항상 표시한다.
...
`lib/aggregate.js` 의 `PLATFORM_BY_REVIEWER` 가 검수자별 플랫폼을 고정한다. **담당자가 바뀌면 이 명단만 수정하면 된다.**

```
무신사 : 이온유, 김혜련, 이용준, 김영조, 윤미지
29cm  : 양호진, 이재승
```

명단에 없는 이름은 기존 방식(UID 접두어: 4=29cm, 그 외=무신사)으로 판별한다.

주의: 이 명단을 적용하면 "플랫폼"의 의미가 *상품이 올라간 사이트*에서 ***검수자 소속***으로 바뀐다. 무신사 UID(5·6번대) 상품을 29cm 담당자가 검수하면 29cm 로 집계된다. 명단 적용 전에는 양호진이 무신사 14건 + 29cm 137건으로 두 줄로 나뉘어 있었고, 적용 후 29cm 151건 한 줄로 합쳐졌다. 총계와 오류 항목별 건수는 영향받지 않는다.

## 색상

- **무신사 = 파랑 `#1877f2`, 29cm = 주황 `#eb6834`**. 선 그래프 계열 색과 표의 플랫폼 배지 색이 같다.
  - 이전의 파랑+보라 조합은 색상 검증에서 하드 FAIL이었다(색맹 ΔE 3.2, 정상 시야 ΔE 14.1 — 하한 15 미달). 파랑+주황은 통과(정상 ΔE 36.7, 색맹 최소 28.9).
- **히트맵 = 빨강 순차 6단계** `#fbdcda → #96201f`. 명도 단조 증가·단계 간격·단일 색조(편차 4°) 검증 통과. 각 단계에서 숫자가 4.5:1 이상 대비를 갖도록 밝은 4단계는 진한 글씨, 어두운 2단계는 흰 글씨를 쓴다. 가장 밝은 단계는 "0에 가까움"을 뜻해 배경으로 물러나며(순차 색상의 정상 동작), 모든 셀에 숫자를 함께 표시하므로 색만으로 읽지 않아도 된다.
- 전체 합계선은 회색 점선이다. 무신사가 전체의 85%라 실선으로는 무신사 선에 완전히 가려진다.
