...
기간 버튼의 `오늘`·`최근 7일`·`최근 30일` 은 **실제 오늘**을 기준으로 계산한다. 정적 스냅샷이라 오늘 데이터가 없을 수 있는데, 그때는 0건으로 표시하고 `이 기간에는 수정요청이 없습니다 (수집 범위 …)` 라고 알려준다.

스냅샷의 기간은 **시작일 = 수정요청이 처음 있는 날, 종료일 = 수집 시점**이다. 종료일을 마지막 데이터가 있는 날로 줄이지 않는 이유는, 그래야 "수집했지만 수정요청이 0건인 날"(주말·휴무 등)이 추이 그래프에 0으로 보이기 때문이다. 시작일만 좁히는 것은 수집 기간(기본 90일) 앞부분의 빈 구간을 없애기 위한 것이다.
